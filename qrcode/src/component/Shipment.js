...
// mydatalist
 ...
 // myundatalist
    ...
    myQRcode(index,item){
      ...
      let IDdweiwancheng=item.id
      //alert(item.id)
      Actions.mydata({listindex});
    }
    myunQRcode(index,item){
      let listindex=this.state.datalist[index];
      let IDdweiwancheng=item.id
      //alert(item.id)
      Actions.myundata({listindex});
    }
    ...
    cancelmy(res){
      ...
      // let timestamp = Date.parse(new Date());
      // axios.get(`${url}/invoice/GetListNoGroup?id=${ID}&&timestamp=${timestamp}`)
      // .then(res=>{this.setState({loading:false,searchlistshow:false,waitdatalistshow:true,alldatalistshow:false,unfinishdatalistshow:false,finishdatalistshow:false,nulldatalistshow:false,datalist:res.data});if (res.data.length>10) {this.setState({nheight:res.data.length})}else{this.setState({nheight:12})};})
      // .catch(err=>console.log(err));
    }
  render(){

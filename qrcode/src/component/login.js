...
class Login extends React.Component {
    ...
    if (res.data.code == 2) {
      for (let i = 0; i < res.data.data.rights.length; i++) {
        if (res.data.data.rights[i].softName == "装车监控软件") {
          ...
          res.data.data.rights[i].softName == "装车发运软件（秦皇岛）"
        ) {
          let jiankong = "false";
          let ID = res.data.data.companyID;
          let renyuandengluid = res.data.data.id;
  ...
  checkbox() {
    ...
    if (flag) {
      ...
      AsyncStorage.setItem("admin", this.state.admin, function (error) {
        console.log(error);
      });
      AsyncStorage.setItem("password", this.state.password, function (error) {
        console.log(error);
      });
    } else {
    ...
    return (
      <View style={styles.flexbox}>
        ...
        <ImageBackground
          ...
          source={require("../img/back1.jpg")}
          resizeMode="cover"
          opacity={1}
        >
          <View style={styles.loginn}>
            <Image
              style={{ height: 0.2 * height, width: 0.4 * width }}
          ...
          <KeyboardAvoidingView behavior="padding" keyboardVerticalOffset={5}>
            <View style={styles.admin}>
              <View>
                <Text style={styles.textfont}>账号:</Text>
              </View>
              <View>
                <TextInput
            ...
            <View style={styles.admin}>
              <View>
                <Text style={styles.textfont}>密码:</Text>
              </View>
              <View>
                <TextInput
          ...
          <View style={styles.admin}>
            <View style={styles.adminbtn}>
              <TouchableNativeFeedback onPress={this.login.bind(this)}>
                <Text style={styles.adminbtntext}>登 录</Text>
              </TouchableNativeFeedback>
            </View>
          </View>
          ...
          <View style={styles.admin}>
            <View style={styles.password}>
              <Text
                style={{ color: "#095194", fontSize: 27 }}
          ...
          <View style={styles.copyright}>
            <Text
              style={{ color: "#eee", textAlign: "center", fontWeight: "bold" }}
              numberOfLines={2}
            >
              Copyright © 2017 Xiaoyu Video Communications (beijing). Technology
              Co.Ltd{" "}
            </Text>
          </View>
        </ImageBackground>
